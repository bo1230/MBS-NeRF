...
class NeRF(nn.Module):
    ...
    def forward(self, barf_i, pts, viewdirs, args):
        ...
        for i, l in enumerate(self.pts_linears):
            h = self.pts_linears[i](h)      # 将点特征输入线性层
            h = F.relu(h)
            if i in self.skips:
                h = torch.cat([input_pts, h], -1)   # 如果是跳跃层的话跳跃连接

        if self.use_viewdirs:
            alpha = self.alpha_linear(h)        # 将点特征放入alpha线性层，得到体密度
            feature = self.feature_linear(h)    # 将点特征放入特征线性层，得到中间特征
            h = torch.cat([feature, input_views], -1)   # 将中间特征和视角特征结合

            for i, l in enumerate(self.views_linears):
                h = self.views_linears[i](h)    # 将结合的特征放入视角线性层
                h = F.relu(h)

            rgb = self.rgb_linear(h)    # 将结合特征放入rgb线性层，得到rgb
            outputs = torch.cat([rgb, alpha], -1)   # 输出rgb和alpha
        else:
...
class Graph(nn.Module):
    ...
    def forward(self, i, depth_gts, img_idx, poses_num, H, W, K, near,far, args, novel_view=False):

        if novel_view:
            rays_depth_coords = 0 # 无关紧要
            poses_sharp = se3_to_SE3(self.se3_sharp.weight)
            ray_idx_sharp = torch.randperm(H * W)[:300]     # 前300条射线
            ret = self.render(i, poses_sharp, ray_idx_sharp, rays_depth_coords, H, W, K, near, far, args)
            return ret, ray_idx_sharp, poses_sharp

        # 获取姿态信息并生成一些随机的射线索引。
        spline_poses = self.get_pose(i, img_idx, args)
        # 只有颜色的随机射线
        ray_rgb_idx = torch.randperm(H * W)[:args.N_rgb_rand]

        rays_depth_coords = []
        rays_depths = []
        rays_weights = []
        for j in img_idx:
            lenth = len(depth_gts[j]['depth'])
            ray_depth_idx = torch.randperm(lenth)[:args.N_depth_rand]
            rays_depth_coord = torch.tensor(depth_gts[j]['coord'][ray_depth_idx.cpu()])
            rays_depth = torch.tensor(depth_gts[j]['depth'][ray_depth_idx.cpu()])
            rays_weight = torch.tensor(depth_gts[j]['error'][ray_depth_idx.cpu()])
            rays_depth_coord = rays_depth_coord.repeat(args.deblur_images, 1)
            rays_depths.append(rays_depth)
            rays_weights.append(rays_weight)
            rays_depth_coords.append(rays_depth_coord)
        rays_depth_coords = torch.cat(rays_depth_coords, dim=0)
        rays_depths = torch.cat(rays_depths, dim=0)
        rays_weights = torch.cat(rays_weights, dim=0)
        ret = self.render(i, spline_poses, ray_rgb_idx, rays_depth_coords, H, W, K, near, far, args,  ray_idx_tv=None, training=True)
        ...
        if (i % args.i_img == 0 or i % args.i_novel_view == 0) and i > 0:
            ...
            return ret, ray_rgb_idx, rays_depths, rays_weights, spline_poses, all_poses
        else:
            return ret, ray_rgb_idx, rays_depths, rays_weights, spline_poses
    ...
    def render(self, barf_i, poses, ray_rgb_idx, rays_depth_coords, H, W, K, near, far, args, ray_idx_tv=None, training=False):
        if training:
            ray_rgb_idx_ = ray_rgb_idx.repeat(poses.shape[0])
            rgb_poses = poses.unsqueeze(1).repeat(1, ray_rgb_idx.shape[0], 1, 1).reshape(-1, 3, 4)
            j = ray_rgb_idx_.reshape(-1, 1).squeeze() // W
            i = ray_rgb_idx_.reshape(-1, 1).squeeze() % W
            rays_o_, rays_d_ = get_specific_rays(i, j, K, rgb_poses)
            rays_o_d = torch.stack([rays_o_, rays_d_], 0)
            rgb_batch_rays = torch.permute(rays_o_d, [1, 0, 2])

            if args.colmap_depth:
                print('get depth rays')
                depth_poses = poses.unsqueeze(1).repeat(1, args.N_depth_rand, 1, 1).reshape(-1, 3, 4)
                rays_depth_o_d = torch.stack(get_rays_by_coord_np(H, W, K[0][0], depth_poses, rays_depth_coords), axis=0)
                depth_batch_rays = torch.permute(rays_depth_o_d, [1, 0, 2])
                batch_rays = torch.cat((rgb_batch_rays, depth_batch_rays), dim=0)
            else:
                batch_rays = rgb_batch_rays
    ...
    @torch.no_grad()
    def render_video(self, barf_i, poses, H, W, K, near, far, args):
        all_ret = {}
        ray_idx = torch.arange(0, H*W)
        rays_depth_coords = 0
        for i in range(0, ray_idx.shape[0], args.chunk):
            ret = self.render(barf_i, poses, ray_idx[i:i+args.chunk], rays_depth_coords, H, W, K, near, far, args)
            for k in ret:
                if k not in all_ret:
                    all_ret[k] = []
