...
def get_rays_by_coord_np(H, W, focal, c2w, coords):
    ...
    i, j = (coords[:, 0] - W) / focal, -(coords[:, 1] - H) / focal
...
def render_video_test(i_, graph, render_poses, H, W, K, near, far, args):
    rgbs = []
    disps = []
    for i, pose in enumerate(tqdm(render_poses)):
        pose = pose[None, :3, :4]
        ret = graph.render_video(i_, pose[:3, :4], H, W, K, near, far, args)
...
def render_image_test(i, graph, render_poses, H, W, K, near, far, args, novel_view=0, need_depth=False):
    if novel_view == 0:
        img_dir = os.path.join(args.basedir, args.expname, 'img_test_{:06d}'.format(i))
    elif novel_view == 1:
        img_dir = os.path.join(args.basedir, args.expname, 'img_novel_1_{:06d}'.format(i))
    elif novel_view == 2:
        img_dir = os.path.join(args.basedir, args.expname, 'img_novel_2_{:06d}'.format(i))
    elif novel_view == 3:
        img_dir = os.path.join(args.basedir, args.expname, 'img_novel_3_{:06d}'.format(i))
